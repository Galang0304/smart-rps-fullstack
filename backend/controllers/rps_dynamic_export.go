...
func addDynamicTugasTables(doc *document.Document, tugasData []interface{}, rps *models.GeneratedRPS, dosens []models.Dosen) {
	...
	for i, item := range tugasData {
		if tugas, ok := item.(map[string]interface{}); ok {
			...
			headerTitlePara := titleCell.AddParagraph()
			headerTitlePara.Properties().SetAlignment(wml.ST_JcCenter)
			headerTitlePara.AddRun().AddText("PERGURUAN TINGGI")
			headerTitlePara.AddRun().AddBreak()
			headerTitlePara.AddRun().AddText("FAKULTAS")
			headerTitlePara.AddRun().AddBreak()
			run := headerTitlePara.AddRun()
			run.AddText("PROGRAM STUDI")
			run.Properties().SetBold(true)
			headerTitlePara.AddRun().AddBreak()
			headerTitlePara.AddRun().AddText("RENCANA TUGAS MAHASISWA")
