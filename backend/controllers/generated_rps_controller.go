...
func (gc *GeneratedRPSController) Export(c *gin.Context) {
	...
	// Tugas - DINAMIS: Hanya buat untuk tugas yang ada
	if tugasData, ok := result["tugas"].([]interface{}); ok {
		numTugas := len(tugasData)
		fmt.Printf("Processing %d tugas items for export\n", numTugas)

		// Iterasi sebanyak jumlah tugas yang ada saja (tidak perlu sampai 20)
		for i := 0; i < numTugas; i++ {
			if tugas, ok := tugasData[i].(map[string]interface{}); ok {
				// Format baru sesuai template yang diberikan
				replaceMap[fmt.Sprintf("{SUB_CPMK_TUGAS_%d}", i+1)] = getString(tugas, "sub_cpmk")
		...
		// JANGAN clear placeholder yang tidak dipakai - biarkan template Word menampilkan kosong
		// Template Word seharusnya hanya berisi tabel sebanyak tugas yang ada
		fmt.Printf("Export will contain %d task tables\n", numTugas)

	} else {
		// Tidak ada tugas sama sekali
		fmt.Println("No tugas data found - export will contain no task tables")
	}
...
// exportWithDynamicTables - Export dengan jumlah tabel dinamis sesuai tugas
func (gc *GeneratedRPSController) exportWithDynamicTables(templatePath, outputPath string, result map[string]interface{}, rps *models.GeneratedRPS, numTugas int) error {
	...
	// Ambil konten XML dari template
	content := doc.Editable()

	// Create placeholder map dengan data lengkap
	replaceMap := gc.createDynamicPlaceholderMap(result, rps, numTugas)

	// Replace all placeholders
	for placeholder, value := range replaceMap {
		content.Replace(placeholder, value, -1)
	}

	// Hapus tabel tugas yang tidak diperlukan dari konten Word
	gc.removeUnusedTaskTables(content, numTugas)

	// Simpan hasil ke file output
	return content.WriteToFile(outputPath)
}

// removeUnusedTaskTables - Hapus tabel untuk tugas yang tidak ada
func (gc *GeneratedRPSController) removeUnusedTaskTables(content *docx.Docx, numTugas int) {
	// Hapus tabel untuk tugas nomor numTugas+1 hingga 20
	for i := numTugas + 1; i <= 20; i++ {
		// Cari dan hapus paragraf yang mengandung header tabel tugas
		taskHeader := fmt.Sprintf("TUGAS %d", i)
		content.Replace(taskHeader, "", -1)
		
		// Hapus juga referensi placeholder yang mungkin tersisa
		content.Replace(fmt.Sprintf("TUGAS_%d", i), "", -1)
	}
	
	fmt.Printf("Removed unused task tables from %d to 20, keeping %d tasks\n", numTugas+1, numTugas)
}

// createDynamicPlaceholderMap - Buat map placeholder hanya untuk tugas yang ada
func (gc *GeneratedRPSController) createDynamicPlaceholderMap(result map[string]interface{}, rps *models.GeneratedRPS, numTugas int) map[string]string {
	replaceMap := make(map[string]string)

	// Data dasar yang selalu ada (wajib diisi semua)
	replaceMap["{NAMA_DOSEN}"] = rps.Dosen.Name
	replaceMap["{KODE_MATA_KULIAH}"] = rps.Course.Code
	replaceMap["{NAMA_MATA_KULIAH}"] = rps.Course.Name
	replaceMap["{SKS}"] = fmt.Sprintf("%d", rps.Course.SKS)
	replaceMap["{SEMESTER}"] = fmt.Sprintf("%d", rps.Course.Semester)
	replaceMap["{PROGRAM_STUDI}"] = rps.Prodi.Name

	// Data umum RPS
	if deskripsi, ok := result["deskripsi"].(string); ok {
		replaceMap["{DESKRIPSI}"] = deskripsi
	}
	
	if capaianPembelajaran, ok := result["capaian_pembelajaran"].(string); ok {
		replaceMap["{CAPAIAN_PEMBELAJARAN}"] = capaianPembelajaran
	}

	if prasyarat, ok := result["prasyarat"].(string); ok {
		replaceMap["{PRASYARAT}"] = prasyarat
	}

	// Tugas dinamis - hanya untuk tugas yang ada
	if tugasData, ok := result["tugas"].([]interface{}); ok {
		for i := 0; i < numTugas && i < len(tugasData); i++ {
			if tugas, ok := tugasData[i].(map[string]interface{}); ok {
				taskNum := i + 1
				
				replaceMap[fmt.Sprintf("{SUB_CPMK_TUGAS_%d}", taskNum)] = getString(tugas, "sub_cpmk")
				replaceMap[fmt.Sprintf("{INDIKATOR_TUGAS_%d}", taskNum)] = getString(tugas, "indikator") 
				replaceMap[fmt.Sprintf("{JUDUL_TUGAS_%d}", taskNum)] = getString(tugas, "judul_tugas")
				replaceMap[fmt.Sprintf("{BATAS_TUGAS_%d}", taskNum)] = getString(tugas, "batas_waktu")
				replaceMap[fmt.Sprintf("{PETUNJUK_TUGAS_%d}", taskNum)] = getString(tugas, "petunjuk_pengerjaan")
				replaceMap[fmt.Sprintf("{LUARAN_TUGAS_%d}", taskNum)] = getString(tugas, "luaran_tugas")
				replaceMap[fmt.Sprintf("{KRITERIA_TUGAS_%d}", taskNum)] = getString(tugas, "kriteria")
				replaceMap[fmt.Sprintf("{TEKNIK_PENILAIAN_TUGAS_%d}", taskNum)] = getString(tugas, "teknik_penilaian")
				replaceMap[fmt.Sprintf("{BOBOT_TUGAS_%d}", taskNum)] = fmt.Sprintf("%v", tugas["bobot"])
				...
				replaceMap[fmt.Sprintf("{DAFTAR_RUJUKAN_%d}", taskNum)] = rujukanText
			}
		}
	}

	// Kosongkan placeholder untuk tugas yang tidak ada (placeholder cleanup)
	for i := numTugas + 1; i <= 20; i++ {
		replaceMap[fmt.Sprintf("{SUB_CPMK_TUGAS_%d}", i)] = ""
		replaceMap[fmt.Sprintf("{INDIKATOR_TUGAS_%d}", i)] = ""
		replaceMap[fmt.Sprintf("{JUDUL_TUGAS_%d}", i)] = ""
	...
	fmt.Printf("Placeholder map created for %d tasks\n", numTugas)
	return replaceMap
}
